...
async function startMetabase(){
  ...
  const metabase = spawn(
    "/usr/bin/java", // full path to Java
  ...
  metabase.on("error",(err)=>{
    console.error("Failed to start Metabase:",err);
  });

  metabase.on("spawn",()=>{
    console.log("Metabase process started");
  });

  metabase.on("exit",(code)=>{
    console.log("Metabase exited with code:",code);
  });
